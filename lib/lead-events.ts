...
export async function recordLeadEvent(
  ...
  opts: { userId: string; leadId: string; type: LeadEventType; metadata?: Record<string, unknown> }
): Promise<void> {
  const { userId, leadId, type, metadata } = opts;
  await client.from("lead_events").insert({ user_id: userId, lead_id: leadId, type, metadata });
}
